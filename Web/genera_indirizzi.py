...
def genera_indirizzi(tipo_miss, size_max, dim_blocco, sets, ways, n_indirizzi):
    ...
    if tipo_miss == "cap":      #se accessi (distanza) > set*way
        if sw < n_indirizzi:
            distanza = random.randint(sw + 2, n_indirizzi)      #CONTROLLARE SE CORRETTO +2
    ...
    #fare lista per controllo in else (riga 65)
    #controllare di introdurre valori con #blocco diverso tra loro
