...
class Cache():
    ...
    #direct mapped     -> 1 way
                    ...
                    if l > 0 and data[l-1][3][i].get_value() == "H":
                        ...
                        if f == 0:
                            ...
                            #PROBLEMA "ELIF"
                                ...
                                - altrimenti: H
                                """
                                if stor_init[data[l][1][i].get_value()][-1] != str(data[l][2][i].get_value()) and str(data[l][2][i].get_value()) not in stor_init[data[l][1][i].get_value()]:
                                    stor_init[data[l][1][i].get_value()].append(str(data[l][2][i].get_value()))

                                    #PROBLEMA (devo controllare storico)
                                    data[l][4][i] = fasi.TipoMiss("L")
                                
                                elif stor_init[data[l][1][i].get_value()][-1] != str(data[l][2][i].get_value()) and str(data[l][2][i].get_value()) in stor_init[data[l][1][i].get_value()]:
                                    stor_init[data[l][1][i].get_value()].append(str(data[l][2][i].get_value()))
                                    ...
                                    for j in range(i-1,-1,-1):
                                        if data[l][1][j].get_value() == data[l][1][i].get_value() and data[l][2][j].get_value() != data[l][2][i].get_value() and data[l][2][j].get_value() not in incontri:
                                            ...
                                            while not trovato or ind_max*(-1) == len(init):
                                                if init[list(init.keys())[ind_max]] != "":
                                                    trovato = True
                                                else:
                                                    ind_max -= 1
                                            ...
                                            for k,v in init.items():
                                                if v != "":
                                                    if int(v)*cache_table.get_sets()[l]+k == data[l][0][i].get_value():
        ...
        if cache_table.get_lvlinit() != 0:
            cachef = dict()
            dataint = data[cache_table.get_lvlinit() - 1]
            for i in range(len(addresses)-1,0,-1):
                if dataint[1][i].get_value() not in cachef:
                    cachef[dataint[1][i].get_value()] = dataint[2][i].get_value()
